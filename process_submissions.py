...
def generate_spreadsheets(
    ...
    for student in student_data.values():
        ...
        graide_data.append([])
        for header in headers:
            lookup_data[len(lookup_data) - 1].append(student[header])
        for header in graide_headers:
            graide_data[len(graide_data) - 1].append(student[header])
    lookup_data_frame = pd.DataFrame(
        lookup_data,
        columns = headers
    )
    graide_data_frame = pd.DataFrame(
        graide_data,
        columns = graide_headers
    )
    with pd.ExcelWriter(file_lookup_name) as writer:
        lookup_data_frame.to_excel(
            writer, 
            startcol = -1, 
            sheet_name = "Anonymised Data"
        )
    
    with pd.ExcelWriter(graide_data_name) as writer:
        graide_data_frame.to_excel(
            writer, 
            startcol = -1, 
            sheet_name = "Graide Formatted"
        )"""
